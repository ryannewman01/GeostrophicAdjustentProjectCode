...
module prob_3D_module
  ...
  public :: amrex_probinit, FORT_INITDATA, initfromrest, &
            ...
            initshearlayer, initiwpctm, initryan,initspectralreadin, initgrimshaw,  initcircular, initgravrelease, &
            FORT_AVERAGE_EDGE_STATES, initstastnafnsgap, &
...
#include <INFL_FORCE_F.H>
      ...
      namelist /fortin/ denerr, vorterr, adverr, temperr, &
     			...
     			rhozero, rhograd, tempzero, c_d, r_d,  &
                       ...
                       deltax, xwidth, omega_0, &
                       rho0, H_m, l_m, eta_power, del_rho, z_0, h, L_z, L_x, kx_store, ky_store, kz_store, &
...
!c      print *, 'INITDATA ', time, lo(1),hi(1), lo(2), hi(2), lo(3), hi(3)
      ...
      if (probtype .eq. 1) then
         ...
         call initryan(level,time,lo,hi,nscal, &
          	          vel,scal,DIMS(state),press,DIMS(press), &
                         dx,xlo,xhi)
      else if (probtype .eq. 32) then
         call initgrimshaw(level,time,lo,hi,nscal, &
          	          vel,scal,DIMS(state),press,DIMS(press), &
                         dx,xlo,xhi)
      else if (probtype .eq. 33) then
         call initcircular(level,time,lo,hi,nscal, &
          	          vel,scal,DIMS(state),press,DIMS(press), &
                         dx,xlo,xhi)
      else if (probtype .eq. 34) then
         call initgravrelease(level,time,lo,hi,nscal, &
          	          vel,scal,DIMS(state),press,DIMS(press), &
                         dx,xlo,xhi)
      else if (probtype .eq. 35) then
         call initdeepwell(level,time,lo,hi,nscal, &
          	          vel,scal,DIMS(state),press,DIMS(press), &
                          dx,xlo,xhi)
      else if (probtype .eq. 36) then
         call initstastna(level,time,lo,hi,nscal, &
          	          vel,scal,DIMS(state),press,DIMS(press), &
                          dx,xlo,xhi)
     else if (probtype .eq. 37) then
         call initspectralreadin(level,time,lo,hi,nscal, &
          	          vel,scal,DIMS(state),press,DIMS(press), &
                          dx,xlo,xhi)
      else if (probtype .eq. 38) then
         call initstastnafnsgap(level,time,lo,hi,nscal, &
              vel,scal,DIMS(state),press,DIMS(press), &
              dx,xlo,xhi)
...
!c ::: -----------------------------------------------------------
!c
      subroutine initryan(level,time,lo,hi,nscal, &
     	 	             vel,scal,DIMS(state),press,DIMS(press), &
                            dx,xlo,xhi)
      implicit none
      ...
      do k = lo(3), hi(3)
         ...
         do j = lo(2), hi(2)
            ...
            do i = lo(1), hi(1)
               ...
               call blutilrand(rn)
               vel(i,j,k,1) = turb_scale*(rn-0.5d0)
               call blutilrand(rn)
               vel(i,j,k,2) = turb_scale*(rn-0.5d0)
               call blutilrand(rn)
               vel(i,j,k,3) = turb_scale*(rn-0.5d0)

               scal(i,j,k,1)= (rho3+rho1)*0.5d0 - (rho3-rho1)*0.5d0*tanh((z-(h3+0.5d0*h2+deltah*tanh((x-deltax)/xwidth)))*sigma)
               
               !if (x.lt.deltax) then
               !   scal(i,j,k,1)= (rho3+rho1)*0.5d0 - (rho3-rho1)*0.5d0*tanh((z-(h3+h2*0.5d0-deltah))*sigma)
               !   scal(i,j,k,2) = one
               !else
               !   scal(i,j,k,1)= (rho3+rho1)*0.5d0 - (rho3-rho1)*0.5d0*tanh((z-(h3+h2*0.5d0))*sigma)
               !   scal(i,j,k,2) = zero
               !endif
...
!c               end do
      ...
      end subroutine initryan
!c
!c ::: -----------------------------------------------------------
!c
      subroutine initgrimshaw(level,time,lo,hi,nscal, &
     	 	             vel,scal,DIMS(state),press,DIMS(press), &
                            dx,xlo,xhi)
      implicit none
      ...
      do k = lo(3), hi(3)
         ...
         do j = lo(2), hi(2)
            ...
            do i = lo(1), hi(1)
               ...
               call blutilrand(rn)
               vel(i,j,k,1) = 0.01d0*(rn-0.5d0)
               call blutilrand(rn)
               vel(i,j,k,2) = 0.01d0*(rn-0.5d0)
               call blutilrand(rn)
               vel(i,j,k,3) = 0.01d0*(rn-0.5d0)
...
!c               end do
      ...
      end subroutine initgrimshaw
!c
!c     
!c
!c ::: -----------------------------------------------------------
!c
      subroutine initcircular(level,time,lo,hi,nscal, &
     	 	             vel,scal,DIMS(state),press,DIMS(press), &
                            dx,xlo,xhi)
      implicit none
...
!c     ::::: local variables
      ...
      REAL_T  hx, hy, hz
      REAL_T  x_vel, y_vel, z_vel
      REAL_T  rn, sigma_inner, sigma_outer
...
#include <probdata.H>
      ...
      hy = dx(2)
      hz = dx(3)
      sigma_inner = 2.64665/(h2_inner*0.5d0)
      sigma_outer = 2.64665/(h2_outer*0.5d0)
      ...
      do k = lo(3), hi(3)
         ...
         do j = lo(2), hi(2)
            ...
            do i = lo(1), hi(1)
               ...
               scal(i,j,k,1) = 0.5d0*( (rho3+rho1) - (rho3-rho1)*tanh( (r-circle_rad)/deltah ) )
               scal(i,j,k,2) = 0.5d0*(     1.d0    -             tanh( (r-circle_rad)/deltah ) )
...
!c               end do
      ...
      end subroutine initcircular
!c
!
!c     ::: -----------------------------------------------------------
!c    
subroutine initgravrelease(level,time,lo,hi,nscal, &
     	 	             vel,scal,DIMS(state),press,DIMS(press), &
                            dx,xlo,xhi)
      implicit none
...
#include <probdata.H>
      ...
      do k = lo(3), hi(3)
         ...
         do j = lo(2), hi(2)
            ...
            do i = lo(1), hi(1)
               ...
               if (x**2+y**2.lt.circle_rad**2) then

                  if (z.lt.h3+h2/2-h2_inner) then
                  scal(i,j,k,1) = rho3
                  scal(i,j,k,2) = zero

                  else if (abs(z-(h3+h2/2)).lt.h2_inner) then
                  scal(i,j,k,1) = (rho3+rho1)/2
                  scal(i,j,k,2) = one
                  ...
                  !     scal(i,j,k,1) = rho1
                  scal(i,j,k,1)= (rho3+rho1)*0.5d0 - (rho3-rho1)*0.5d0*tanh((z-(h3+h2*0.5d0))*sigma_outer)
                  scal(i,j,k,2) = zero
               endif
      ...
      end subroutine initgravrelease
!c
!
!c     ::: -----------------------------------------------------------
!c
      subroutine initdeepwell(level,time,lo,hi,nscal, &
     	 	             vel,scal,DIMS(state),press,DIMS(press), &
                            dx,xlo,xhi)
      implicit none
...
#include <probdata.H>
      ...
      do k = lo(3), hi(3)
         ...
         do j = lo(2), hi(2)
            ...
            do i = lo(1), hi(1)
               ...
               eta = (H_m/2)*exp(-(x/L_m)**eta_power)  

              scal(i,j,k,1) = rho0 - (del_rho/4)*(tanh((z-z_0-eta)/h) + tanh((z-z_0+eta)/h))

              if ((x.lt.L_m).and.(abs(scal(i,j,k,1)-rho0).lt.(del_rho/4))) then
              scal(i,j,k,2)=one
              else
              scal(i,j,k,2)=zero
              endif
    ...
    end subroutine initdeepwell

!c     ::: -----------------------------------------------------------
!c
      subroutine initstastna(level,time,lo,hi,nscal, &
     	 	             vel,scal,DIMS(state),press,DIMS(press), &
                            dx,xlo,xhi)
      implicit none
...
!c     ::::: local variables
      ...
      REAL_T  x_vel, y_vel, z_vel
      REAL_T  rn, sigma, lambda, xi
      REAL_T x_1, x_2, z_1, z_2
      REAL_T rho_bar, rho_m
...
#include <probdata.H>
      ...
      do k = lo(3), hi(3)
         ...
         do j = lo(2), hi(2)
            ...
            do i = lo(1), hi(1)
               ...
               rho_bar = 1 - 0.01*tanh((z +z_0)/(0.05*L_z))
               rho_m = 0.5*(tanh((x+x_1)/lambda) + tanh((x+x_2)/lambda) + tanh((z+z_1)/xi) + tanh((z+z_2)/xi))

             
              scal(i,j,k,1) = rho_bar + rho_m
              
              !if ((x.lt.0.25).and.(abs(z-z_0).lt.2*h)) then
              !scal(i,j,k,2)=one
              !else
              !scal(i,j,k,2)=zero
              !endif
    ...
    end subroutine initstastna

    !c ::: -----------------------------------------------------------
!c
      subroutine initstastnafnsgap(level,time,lo,hi,nscal, &
     	 	             vel,scal,DIMS(state),press,DIMS(press), &
                            dx,xlo,xhi)
      implicit none
...
#include <probdata.H>
      ...
      do k = lo(3), hi(3)
         ...
         do j = lo(2), hi(2)
            ...
            do i = lo(1), hi(1)
               ...
               eta = 0.05*exp(- ((x-deltax)/omega_0)**8)
               scal(i,j,k,1)= 1.0 
      ...
      end subroutine initstastnafnsgap
!c

!c ::: -----------------------------------------------------------
    !c
    !c Attempt at reading in conditions from spectral code
      subroutine initspectralreadin(level,time,lo,hi,nscal, &
     	 	             vel,scal,DIMS(state),press,DIMS(press), &
                            dx,xlo,xhi)
      implicit none
...
!c     ::::: local variables
      ...
      real, allocatable, dimension(:,:,:) :: rho_store
#include <probdata.H>

      open(17,file='SpectralCodeDensity.dat')

      allocate(rho_store(kx_store,ky_store,kz_store))

      do k = 1,kz_store
         do j = 1,ky_store
            do i = 1,kx_store
               read(17,*) scal(i,j,k,1)
            end do
         end do
         !print*, k
      end do
...
!c               end do
      ...
      end subroutine initspectralreadin
!c
!c ::: -----------------------------------------------------------
...
!!$omp&private(xT,kappa)
            ...
            enddo
#endif
         else if ( (probtype == 19) .or. (probtype ==31) .or. (probtype == 32) .or. (probtype == 33) .or. (probtype == 34) .or. (probtype == 35)) then
            ! Coriolis
            do k = f_lo(3), f_hi(3)
               z = xlo(3) + hz*(float(k-f_lo(3)) + half)
               do j = f_lo(2), f_hi(2)
